	...
	<fieldset class="formSep">
		<div class="row-fluid editpage-rightbar">
			...
			<div class="right-bar move-mod">
								...
								<div class="ecjiaf-db">
									<div class="fileupload {if $cat_info.style}fileupload-exists{else}fileupload-new{/if} m_t10" data-provides="fileupload">
										...
										<div>
											...
											<a class="btn fileupload-exists" data-toggle="removefile" data-msg="{t}您确定要删除此分类图片吗？{/t}" data-href='{url path="goods/admin_category/remove_logo" args="cat_id={$cat_info.cat_id}"}' {if $cat_info.style}data-removefile="true"{/if}>{t}删除{/t}</a>
										</div>
									</div>
								</div>
							</div>
				...
				<div class="foldable-list move-mod-group" id="admin_category_credentials">
					<div class="accordion-group">
						<div class="accordion-heading">
							<a class="accordion-toggle collapsed move-mod-head" data-toggle="collapse" data-target="#category_credentials">
								<strong>{t}类目证件{/t}</strong>
							</a>
						</div>
						<div class="accordion-body in collapse" id="category_credentials">
							<div class="accordion-inner">
								<div class="control-group control-group-small">
									<label class="w80 m_l5 fl t_l"><b>{t}证件标题：{/t}</b></label>
									<!-- {foreach from=$title_list item=credentials name="foreach_credentials"}-->
									...
									<!-- {/foreach} -->
									<a class="tigger_add m_l5 l_h30" href="javascript:;">{t}再添加一项{/t}</a>
								</div>
							</div>
						</div>
					</div>
