	...
	<li class="ecjiaf-fn">
		<form name="searchForm" method="post" action="{$form_search}{if $filter.type}&type={$filter.type}{/if}">
			<div class="f_r form-inline">
				<input type="text" name="merchant_keywords" value="{$smarty.get.merchant_keywords}" placeholder="{lang key='goods::goods_type.merchant_keywords'}"/> 
				<input type="text" name="keywords" value="{$smarty.get.keywords}" placeholder="{lang key='goods::goods_type.goods_type_keywords'}"/> 
				<button class="btn" type="submit">{lang key='goods::goods_type.search'}</button>
			</div>
		</form>
	</li>
</ul>
		...
		<table class="table table-striped dataTable table-hide-edit">
			<thead>
				<tr>
					...
					<th class="w130">{lang key='goods::goods_type.merchants_name'}</th>
					<th>{lang key='goods::goods_type.attr_groups'}</th>
			...
			<tbody>
				...
				<tr>
					...
					<td class="ecjiafc-red">{$goods_type.merchants_name}</td>
					<td>{$goods_type.attr_group}</td>
					<td>{$goods_type.attr_count}</td>
					<td><i class="{if $goods_type.enabled}fontello-icon-ok cursor_pointer{else}fontello-icon-cancel cursor_pointer{/if}" title="{lang key='goods::goods_type.click_edit_stats'}" data-trigger="toggleState" data-url="{RC_Uri::url('goods/admin_goods_type/toggle_enabled')}" data-id="{$goods_type.cat_id}"></i></td>
				</tr>
