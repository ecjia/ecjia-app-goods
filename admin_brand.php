...
class admin_brand extends ecjia_admin 
{
	...
	public function insert() 
	{
    	...
    	$link[0]['href'] = RC_Uri::url('goods/admin_brand/add');
    
    	$link[1]['text'] = RC_Lang::lang('back_list');
    	$link[1]['href'] = RC_Uri::url('goods/admin_brand/init');
...
// 		$brand_arr['brand_local'] = strpos($brand_arr['brand_logo'], 'http://') === false ? 0 : 1 ;
		ecjia_screen::get_current_screen()->add_nav_here(new admin_nav_here(__('自营品牌') , RC_Uri::url('goods/admin_brand/init')));
	...
	public function update() 
	{
	    ...
	    /* 对编辑品牌进行权限检查  BY：MaLiuWei  START */
	    if (!empty($_SESSION['ru_id'])) {
	    	$this->showmessage(__('入驻商家没有操作权限，请登陆商家后台操作！'), ecjia::MSGTYPE_JSON | ecjia::MSGSTAT_ERROR);
	    }
	    /* 对编辑品牌进行权限检查  BY：MaLiuWei  END */
	...
	public function toggle_show()
	{
		...
		/* 对切换进行权限检查  BY：MaLiuWei  START */
		if (!empty($_SESSION['ru_id'])) {
			$this->showmessage(__('入驻商家没有操作权限，请登陆商家后台操作！'), ecjia::MSGTYPE_JSON | ecjia::MSGSTAT_ERROR);
		}
		/* 对切换进行权限检查  BY：MaLiuWei  END */
		$id     = intval($_POST['id']);
