...
class StoreCashierGoodsDuplicate extends StoreDuplicateAbstract
{
    ...
    {
        $this->name = __('在售收银台商品', 'goods');

        parent::__construct($store_id, $source_store_id);

        $this->data_operator = RC_DB::table('goods')->where('store_id', $this->source_store_id)->where('is_on_sale', 1)->where('is_delete', '!=', 1)->where('extension_code', 'cashier');
    ...
    {
        $text = sprintf(__('店铺内总共有<span class="ecjiafc-red ecjiaf-fs3">%s</span>件收银台商品', 'goods'), $this->handleCount());
