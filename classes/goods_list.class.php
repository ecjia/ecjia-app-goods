...
class goods_list {
	...
	public static function get_goods_list($filter) {
		...
		$field = "g.goods_id, g.goods_name, g.goods_sn, g.store_id, g.goods_name_style, g.comments_number, g.market_price,
...
// 	        }
	 ...
	 * @param   object  $filters    过滤条件
	 */
	public static function get_product_goods_list($filter) {
	    RC_Loader::load_app_class('goods_category', 'goods', false);
	    $dbview = RC_Model::model('goods/goods_member_viewmodel');
	    $dbview->view = array(
	        'store_franchisee' => array(
	    ...
	    $field = "g.goods_id, g.goods_name, g.goods_sn, g.store_id, g.goods_name_style, g.market_price,
				  ...
				  g.goods_brief, g.goods_thumb, g.original_img, g.goods_img,
				  sf.merchants_name,sf.manage_mode,
				  p.product_sn, p.product_number, p.goods_attr";
	    $where = array(
	        'is_on_sale'	=> 1,
	        'is_alone_sale' => 1,
	        'is_delete'		=> 0,
	    ...
	    if (empty($goods_result['list'])) {
	        ...
	        if (!empty($data)) {
	            ...
	            foreach ($data as $key => $row) {
	                ...
	                $arr[$key]['name']			= $row['goods_name'];
	                $arr[$key]['goods_sn']		= $row['goods_sn'];
	                $arr[$key]['product_sn']		= $row['product_sn'];
	                $arr[$key]['product_number']	= $row['product_number'];
	                $arr[$key]['goods_attr_id']		= $row['goods_attr'];
	                $arr[$key]['goods_attr_name']	= $row['goods_attr_name'];
	                $arr[$key]['goods_brief'] 	= $row['goods_brief'];
	                $arr[$key]['store_id']		= $row['store_id'];
	                $arr[$key]['store_name']	= $row['merchants_name'];
	                $arr[$key]['store_logo']	= $store_logo;
