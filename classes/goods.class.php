...
class goods {
    ...
    public static function goods_list($is_delete, $real_goods = 1, $conditions = '') {
        ...
        $filter ['sort_by'] = empty ($_REQUEST ['sort_by']) ? 'g.sort_order' : trim($_REQUEST ['sort_by']);
        ...
        $field = 'g.user_id, goods_id, goods_name, goods_type, goods_sn, review_status, shop_price, goods_thumb, is_on_sale, is_best, is_new, is_hot, g.sort_order, goods_number, integral,(promote_price > 0 AND promote_start_date <= ' . $today . ' AND promote_end_date >= ' . $today . ')|is_promote, ms.shopNameSuffix, ms.shoprz_brandName';
        $sql = $db->field($field)->join('merchants_shop_information')->where('is_delete = ' . $is_delete . '' . $where)->order(array($filter['sort_by'] => $filter ['sort_order']))->limit($page->limit())->select();
