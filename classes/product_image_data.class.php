...
class product_image_data {
	 ...
	 */
	public function __construct($file_name, $file_path, $img_ext, $goods_id, $product_id) {
	    ...
	    $this->goods_format = new goods_image_format($product_id, $img_ext, goods_image_format::PRODUCT_IMAGE);
	    $this->gallery_format = new goods_image_format($product_id, $img_ext, goods_image_format::PRODUCT_GALLERY);
	...
	public function update_goods($img_desc = null) {
	    ...
	    /* 如果有上传图片，删除原来的商品图 */
	    $row = RC_DB::table('products')->where('product_id', $this->product_id)->first();

	    $data = array(
			'product_img'      => $goods_img,
	        'product_thumb'    => $goods_thumb,
		...
		if ($row['product_thumb']) {
		    goods_imageutils::deleteImage(goods_imageutils::getAbsolutePath($row['product_thumb']));
		}
		if ($row['product_img']) {
		    goods_imageutils::deleteImage(goods_imageutils::getAbsolutePath($row['product_img']));
		}
		if ($row['product_original_img']) {
		    goods_imageutils::deleteImage(goods_imageutils::getAbsolutePath($row['product_original_img']));
		}
...
// 	    goods_imageutils::deleteImage($this->file_path);
	    ...
	    if (!ecjia::config('retain_original_img') && !empty($goods_original)) {
            RC_DB::table('products')->where('product_id', $this->product_id)->update(array('product_original_img' => ''));
	...
	public function update_thumb() {
	    ...
	    $data = array('product_thumb' => $goods_thumb);
        RC_DB::table('products')->where('product_id', $this->product_id)->update($data);
...
// 	    goods_imageutils::deleteImage($this->file_path);
	    ...
	    if (!ecjia::config('retain_original_img') && !empty($data['img_original'])) {
	        $db_goods_gallery->where(array('product_id' => $this->product_id))->update(array('img_original' => ''));
