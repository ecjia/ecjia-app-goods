...
class admin_goods_auto extends ecjia_admin {
	...
	public function batch_start() {
		...
		if (empty($goods_id)) {
			$this->showmessage(RC_Lang::get('goods::goods_auto.select_start_goods'), ecjia::MSGTYPE_JSON | ecjia::MSGSTAT_ERROR);
		}
		 
		if (empty($time)) {
			$this->showmessage(RC_Lang::get('goods::goods_auto.select_time'), ecjia::MSGTYPE_JSON | ecjia::MSGSTAT_ERROR);
		}
		...
		$this->showmessage(RC_Lang::get('goods::goods_auto.batch_start_succeed'), ecjia::MSGTYPE_JSON | ecjia::MSGSTAT_SUCCESS, array('pjaxurl' => RC_Uri::url('goods/admin_goods_auto/init')));
	}
	...
	public function batch_end() {
		...
		if (empty($goods_id)) {
			$this->showmessage(RC_Lang::get('goods::goods_auto.select_end_goods'), ecjia::MSGTYPE_JSON | ecjia::MSGSTAT_ERROR);
		}
		...
		$this->showmessage(RC_Lang::get('goods::goods_auto.batch_end_succeed'), ecjia::MSGTYPE_JSON | ecjia::MSGSTAT_SUCCESS, array('pjaxurl' => RC_Uri::url('goods/admin_goods_auto/init')));
	}
	...
	public function del() {
		...
		$this->showmessage(RC_Lang::get('goods::goods_auto.delete_ok'), ecjia::MSGTYPE_JSON | ecjia::MSGSTAT_SUCCESS);
	}
	...
	public function edit_starttime() {
		...
		if (!preg_match('/^\d{4}-\d{2}-\d{2}$/', $value) || $value == '0000-00-00' || $val <= 0) {
			$this->showmessage(RC_Lang::get('goods::goods_auto.time_format_error'), ecjia::MSGTYPE_JSON | ecjia::MSGSTAT_ERROR);
		}
		...
		$this->showmessage('', ecjia::MSGTYPE_JSON | ecjia::MSGSTAT_SUCCESS, array('pjaxurl' => RC_Uri::url('goods/admin_goods_auto/init')));
	}
