...
class admin_goods_booking extends ecjia_admin 
{
	...
	public function remove() 
	{
		...
		$this->showmessage(RC_Lang::lang('js_languages/drop_success') , ecjia_admin::MSGTYPE_JSON | ecjia_admin::MSGSTAT_SUCCESS );
	}
	...
	public function update() 
	{
		...
		/* 对处理提交数据进行权限检查  BY：MaLiuWei  START */
		if (!empty($_SESSION['ru_id'])) {
			$this->showmessage(__('入驻商家没有操作权限，请登陆商家后台操作！'), ecjia::MSGTYPE_JSON | ecjia::MSGSTAT_ERROR);
		}
		/* 对处理提交数据进行权限检查  BY：MaLiuWei  END */
		...
		if (!empty($_POST['send_email_notice']) or isset($_POST['remail'])) {
			...
			if (RC_Mail::send_mail($booking_info['link_man'], $booking_info['email'], $template['template_subject'], $content, $template['is_html'])) {
				$this->showmessage('邮件发送成功！' , ecjia_admin::MSGTYPE_JSON | ecjia_admin::MSGSTAT_SUCCESS , array('pjaxurl' => RC_Uri::url('goods/admin_goods_booking/detail','id='.$_POST['rec_id'])));
			} else {
				$this->showmessage(RC_Lang::lang('mail_send_fail') , ecjia_admin::MSGTYPE_JSON | ecjia_admin::MSGSTAT_ERROR );
			}
		} else {
			if (empty($dispose_note)) {
				$this->showmessage(RC_Lang::lang('js_languages/no_note') , ecjia_admin::MSGTYPE_JSON | ecjia_admin::MSGSTAT_ERROR);
			}
			...
			if ($this->db_book->where(array('rec_id' => $_POST['rec_id']))->update($data)) {
				...
				$this->showmessage(RC_Lang::lang('dispose_succeed') , ecjia_admin::MSGTYPE_JSON | ecjia_admin::MSGSTAT_SUCCESS , array('pjaxurl' => RC_Uri::url('goods/admin_goods_booking/detail','id='.$_POST['rec_id'])));
			}
		}
