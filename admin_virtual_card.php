...
class admin_virtual_card extends ecjia_admin {
	...
	public function edit_replenish() {
		...
		$batch = !empty($_GET['batch']) ? $_GET['batch'] : '';
		/* 判断是批量编辑还是单个编辑  */
		if (!$batch) {
			/* 获取卡片信息 */
	...
	public function insert_replenish() {
		...
		$card_sn_Array 			= !empty($_POST['card_sn']) ? $_POST['card_sn'] : '';
		$card_password_Array 	= !empty($_POST['card_password']) ? $_POST['card_password'] : '';
		$end_date_Array			= !empty($_POST['end_date']) ? $_POST['end_date'] : '';

		/* 遍历验证是否为空 */
		$newArray = array();
		$i = 0 ;
		if (!empty($card_sn_Array)) {
			foreach ($card_sn_Array as $k => $v) {
				...
				if ($this->db_virtual_card->where(array('goods_id' => $goods_id, 'card_sn' => RC_Crypt::encrypt($v, $this->auth_key)))->count() == 0) {
					...
					/* 如果添加成功且原卡号为空时商品库存加1 */
					$this->db_goods->inc('goods_number', 'goods_id='.$goods_id, 1);
				} else {
	...
	public function update_replenish() {
		...
		$goods_id		= !empty($_POST['goods_id']) ? intval($_POST['goods_id']) : 0;
		$card_sn		= !empty($_POST['card_sn']) ? $_POST['card_sn'] : '';
		$card_id		= !empty($_POST['card_id']) ? $_POST['card_id'] : '';
		$card_password	= !empty($_POST['card_password']) ? $_POST['card_password'] : '';
		$end_date		= !empty($_POST['end_date']) ? $_POST['end_date'] : '';
		$batch			= !empty($_POST['batch']) ? $_POST['batch'] : '';
		...
		if (!$batch) {
			...
			if ($card_sn[0] != $_POST['old_card_sn']) {
				if ($this->db_virtual_card->where(array('goods_id' => $goods_id, 'card_sn' => $coded_card_sn))->count() > 0) {
					$this->showmessage(sprintf(RC_Lang::lang('card_sn_exist'),"[ ".$card_sn[0]." ]"), ecjia::MSGTYPE_JSON | ecjia::MSGSTAT_ERROR);
				}
			}
