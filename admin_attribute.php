...
class admin_attribute extends ecjia_admin {
	public function __construct() {
		...
		ecjia_screen::get_current_screen()->add_nav_here(new admin_nav_here(__('商品规格列表', 'goods'), RC_Uri::url('goods/admin_goods_spec/init')));
	}
	...
	public function init() {
		...
		ecjia_screen::get_current_screen()->add_nav_here(new admin_nav_here(__('商品属性', 'goods')));
		ecjia_screen::get_current_screen()->add_help_tab(array(
			'id'		=> 'overview',
			'title'		=> __('概述', 'goods'),
			'content'	=> '<p>' . __('欢迎访问ECJia智能后台商品属性列表页面，系统中所有的商品属性都会显示在此列表中。', 'goods'). '</p>'
		));
		
		ecjia_screen::get_current_screen()->set_help_sidebar(
			'<p><strong>' . __('更多信息：', 'goods') . '</strong></p>' .
			'<p>' . __('<a href="https://ecjia.com/wiki/帮助:ECJia智能后台:商品类型#.E5.B1.9E.E6.80.A7.E5.88.97.E8.A1.A8" target="_blank">关于商品属性列表帮助文档</a>', 'goods') . '</p>'
		);
		...
		$this->assign('ur_here', __('商品属性', 'goods'));
		$this->assign('action_link', array('href' => RC_Uri::url('goods/admin_attribute/add', array('cat_id' => $goods_type)), 'text' => __('添加属性', 'goods')));
		$this->assign('action_link2', array('text' => __('商品规格列表', 'goods'), 'href' => RC_Uri::url('goods/admin_goods_spec/init')));
	...
	public function add() {
		...
		ecjia_screen::get_current_screen()->add_nav_here(new admin_nav_here(__('商品属性', 'goods'), RC_Uri::url('goods/admin_attribute/init', 'cat_id='.$cat_id)));
		ecjia_screen::get_current_screen()->add_nav_here(new admin_nav_here(__('添加属性', 'goods')));
	
		ecjia_screen::get_current_screen()->add_help_tab(array(
			'id'		=> 'overview',
			'title'		=> __('概述', 'goods'),
			'content'	=> '<p>' . __('欢迎访问ECJia智能后台添加商品属性页面，可以在此页面添加商品属性信息。', 'goods') . '</p>'
		));
		...
		$this->assign('action_link', array('href' => RC_Uri::url('goods/admin_attribute/init', 'cat_id='.$cat_id), 'text' => __('商品属性', 'goods')));
	...
	public function insert() {
		...
		if (empty($cat_id)) {
			return $this->showmessage(__('商品类型不能为空', 'goods'), ecjia::MSGTYPE_JSON | ecjia::MSGSTAT_ERROR);
		}
		...
		if ($attr_id) {
			...
			$links = array(
				array('text' => __('返回属性列表', 'goods'), 'href' => RC_Uri::url('goods/admin_attribute/init', array('cat_id' => $cat_id))),
				array('text' => __('添加下一个属性', 'goods'), 'href' => RC_Uri::url('goods/admin_attribute/add', array('cat_id' => $cat_id))),
			);
			return $this->showmessage(sprintf(__('添加属性 [%s] 成功。', 'goods'), $attr['attr_name']), ecjia::MSGTYPE_JSON | ecjia::MSGSTAT_SUCCESS, array('pjaxurl' => RC_Uri::url('goods/admin_attribute/edit', array('attr_id' => $attr_id)), 'links' => $links));
		}
	}
	...
	public function edit() {
		...
		/* 模板赋值 */
		ecjia_screen::get_current_screen()->add_nav_here(new admin_nav_here(__($is_add ? __('添加属性', 'goods') : __('编辑属性', 'goods'))));
		$this->assign('ur_here', $is_add ? __('添加属性', 'goods') : __('编辑属性', 'goods'));
		$this->assign('action_link', array('href' => RC_Uri::url('goods/admin_attribute/init', array('cat_id' => $attr['cat_id'])), 'text' => __('商品属性', 'goods')));
		$this->assign('form_action', RC_Uri::url('goods/admin_attribute/update'));
		
		ecjia_screen::get_current_screen()->add_help_tab(array(
			'id'		=> 'overview',
			'title'		=> __('概述', 'goods'),
			'content'	=> '<p>' . __('欢迎访问ECJia智能后台编辑商品属性页面，可以在此页面编辑商品属性信息。', 'goods') . '</p>'
		));
		
		ecjia_screen::get_current_screen()->set_help_sidebar(
			'<p><strong>' . __('更多信息：', 'goods') . '</strong></p>' .
			'<p>' . __('<a href="https://ecjia.com/wiki/帮助:ECJia智能后台:商品类型#.E7.BC.96.E8.BE.91.E5.B1.9E.E6.80.A7" target="_blank">关于编辑商品属性帮助文档</a>', 'goods') . '</p>'
		);
	...
	public function update() {
		...
		return $this->showmessage(sprintf(__('编辑属性 [%s] 成功。', 'goods'), $attr['attr_name']), ecjia::MSGTYPE_JSON | ecjia::MSGSTAT_SUCCESS, array('links' => $links, 'pjaxurl' => RC_Uri::url('goods/admin_attribute/edit', array('attr_id' => $attr_id))));
	}
	...
	public function batch() {
		...
		if (isset($_POST['checkboxes'])) {
			...
			return $this->showmessage(sprintf(__('成功删除了 %d 条商品属性', 'goods'), $count), ecjia::MSGTYPE_JSON | ecjia::MSGSTAT_SUCCESS, array('pjaxurl' => RC_Uri::url('goods/admin_attribute/init', array('cat_id' => $cat_id))));
		}
	}
	...
	public function edit_attr_name() {
		...
		if (!empty($val)) {
			...
			if (RC_DB::table('attribute')->where('attr_id', $id)->update($data)) {
				...
				return $this->showmessage(__('编辑成功', 'goods'), ecjia::MSGTYPE_JSON | ecjia::MSGSTAT_SUCCESS, array('content' => $val));
			}
		} else {
