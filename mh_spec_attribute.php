...
class mh_spec_attribute extends ecjia_merchant {
	public function __construct() {
		...
		ecjia_merchant_screen::get_current_screen()->set_parentage('goods', 'goods/mh_spec_attribute.php');
	}
	...
	public function init() {
		...
		ecjia_merchant_screen::get_current_screen()->add_nav_here(new admin_nav_here(__('属性列表', 'goods')));
	...
	public function add() {
		...
		$cat_id = isset($_GET['cat_id']) ? intval($_GET['cat_id']) : 0;
		ecjia_merchant_screen::get_current_screen()->add_nav_here(new admin_nav_here(__('规格属性', 'goods'), RC_Uri::url('goods/mh_spec_attribute/init', array('cat_id' => $cat_id))));
	...
	public function edit() {
		...
		ecjia_merchant_screen::get_current_screen()->add_nav_here(new admin_nav_here(__('编辑属性', 'goods')));
	...
	public function update() {
		...
		$cat_id  = !empty($_POST['spec_cat_id']) ? intval($_POST['spec_cat_id']) : 0;
		$attr_id = !empty($_POST['attr_id']) 	 ? intval($_POST['attr_id'])     : 0;
		
		if (RC_DB::table('attribute')->where('cat_id', $cat_id)->where('attr_name', trim($_POST['attr_name']))->where('attr_id', '!=', $attr_id)->count()) {
			return $this->showmessage(__('属性名称在当前规格模板下已存在，请您换一个名称', 'goods'), ecjia::MSGTYPE_JSON | ecjia::MSGSTAT_ERROR);
		}
		...
		return $this->showmessage(sprintf(__('编辑规格属性 [%s] 成功。', 'goods'), $attr['attr_name']), ecjia::MSGTYPE_JSON | ecjia::MSGSTAT_SUCCESS, array('pjaxurl' => RC_Uri::url('goods/mh_spec_attribute/edit', array('attr_id' => $attr_id))));
	}
	...
	public function batch() {
		...
		if (isset($_POST['checkboxes'])) {
			...
			return $this->showmessage(sprintf(__('成功删除了 %d 条商品属性', 'goods'), $count), ecjia::MSGTYPE_JSON | ecjia::MSGSTAT_SUCCESS, array('pjaxurl' => RC_Uri::url('goods/mh_spec_attribute/init', array('cat_id' => $cat_id))));
		}
	}
