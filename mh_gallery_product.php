 ...
 *  ECJIA 商品相册管理
 */
class mh_gallery_product extends ecjia_merchant {
	...
	public function __construct() {
        ...
        ecjia_merchant_screen::get_current_screen()->set_parentage('goods', 'goods/mh_gallery_product.php');
    ...
    public function init() {
        ...
        /* 图片列表 */
        $img_list = RC_DB::table('goods_gallery')->where('goods_id', $goods_id)->where('product_id', $product_id)->get();
        ...
        if (isset($_GET['step'])) {
        	$this->assign('step', 4);
        	$this->assign('url', RC_Uri::url('goods/merchant/edit_link_goods', "goods_id=$goods_id".'&id='.$product_id  ));
        	$this->assign('form_action', RC_Uri::url('goods/mh_gallery_product/insert', "goods_id=".$goods_id.'&id='.$product_id));
        } else {
        	$this->assign('form_action', RC_Uri::url('goods/mh_gallery_product/insert', "goods_id=$goods_id".'&id='.$product_id));
        }
    ...
    public function insert() {
        ...
        $url = RC_Uri::url('goods/mh_gallery_product/init', $arr);
