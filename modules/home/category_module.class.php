 ...
 * @author royalwang
 */
class home_category_module extends api_front implements api_interface {
	public function handleRequest(\Royalcms\Component\HttpKernel\Request $request) {
        ...
        if (! empty($category)) {
            foreach ($category as $key => $val) {
                ...
                $goods = $this->EM_get_category_recommend_goods('best', $children);
                if (empty($goods)) {
                	continue;
                }
                $ngoods = array();
...
// 						}
    ...
    private function EM_get_category_recommend_goods($type = '', $cats = '', $brand = 0, $min = 0, $max = 0, $ext = '') {
    	$where = array();
