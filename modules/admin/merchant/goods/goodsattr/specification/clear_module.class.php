 ...
 *
 */
class admin_merchant_goods_goodsattr_specification_clear_module extends api_admin implements api_interface {
    public function handleRequest(\Royalcms\Component\HttpKernel\Request $request) {
    	...
    	$product_list = RC_DB::table('products')->where('goods_id', $goods_id)->get();
    	...
    	if($count > 0) {
    		return new ecjia_error('not_exists_info', __('该规格模板下的货品已产生了订单交易，不可以更改规格模板', 'goods'));
    	} else {
    		//清除商品关联的模板id为0
    		...
    		foreach($product_list as $value) {
    			$image_data = new product_image_data('', '', '', 0, $value['product_id']);
    			$image_data->delete_images();
    			$result = RC_DB::table('products')->where('product_id', $value['product_id'])->delete();
    			if ($result) {
    				$image_data->delete_gallery();
    				RC_DB::table('goods_gallery')->where('goods_id', $value['goods_id'])->where('product_id', $value['product_id'])->delete();
    			}
    		}
    	}
