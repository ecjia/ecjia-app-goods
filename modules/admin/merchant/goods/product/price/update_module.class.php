 ...
 *
 */
class admin_merchant_goods_product_price_update_module extends api_admin implements api_interface {
    public function handleRequest(\Royalcms\Component\HttpKernel\Request $request) {
    	...
    	if ($products_array) {
    		foreach ($products_array as $val) {
    			if (empty($val['product_id']) || !is_numeric($val['price'])) {
    				return new ecjia_error('edited_product_info_error', __('要修改的货品参数错误', 'goods'));
    			}
    			$price = floatval($val['price']);
    			Ecjia\App\Goods\Models\ProductsModel::where('goods_id', $goods_id)->where('product_id', $val['product_id'])->update(array('product_shop_price' => $price));
    		}
    	}
