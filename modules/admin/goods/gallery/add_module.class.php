...
// 		}
		...
		if (!empty($_FILES)) {
			...
			//最多可传5张图片
			if ($count > 5) {
				return new ecjia_error('goods_gallery_error', __('商品相册图片不能超过5张！', 'goods'));
			}
			$images = $_FILES['image'];
			...
			//最多可传5张图片
			if ($count > 5) {
				return new ecjia_error('goods_gallery_error', __('商品相册图片不能超过5张！', 'refund'));
			}
