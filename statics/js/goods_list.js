...
;(function(app, $) {
		...
		checkGoodsSn : function() {
			$('[data-toggle="checkGoodsSn"]').on('blur', function(e) {
				...
				var $this		= $(this),
					...
					goods_sn	= $this.val() || '',
					info 		= {goods_id : goods_id, goods_sn : goods_sn};
	...
	app.products_list = {
		...
		products_submit : function() {
			...
			var option = {
				rules:{
					product_sn 		: {required : true},
					product_number 	: {required : true}
				},
				messages:{
					product_sn 		: {required : "请输入货号"},
					product_number 	: {required : "请输入库存"}
				},
				submitHandler:function(){
					$this.ajaxSubmit({
						dataType:"json",
		...
		init : function() {
			...
			$('[data-trigger="toggleSpec"]').on('click', function(){
				...
				if ($this.find('i').hasClass('fontello-icon-minus')){
					...
					info.find('select').attr({'id' : '', 'class' : ''}).chosen();
				}
			})
	...
	app.link_goods = {
		...
		change_link_model : function() {
			...
			$(document).off('click.changelinksmod').on('click.changelinksmod', '.change_links_mod', function() {
				...
				$info.attr('data-double', '0').parents('.ms-elem-selection').find('.link_static').text('[单向关联]') :
				$info.attr('data-double', '1').parents('.ms-elem-selection').find('.link_static').text('[双向关联]');
			});
		},
	...
	app.link_area = {
		...
		search_link_area : function() {
			/* 查找商品 */
			$('[data-toggle="searcharea"]').on('click', function() {
				var $choose_list = $('.choose_list'),
					searchURL = $choose_list.attr('data-url'),
					ra_id = $('.link_area').val();
				$.post(searchURL,{'ra_id' : ra_id}, function(data) {
	...
	app.link_article = {
		...
		search_article : function() {
			...
			$('[data-toggle="searchArticle"]').on('click', function() {
				var $choose_list 	= $('.choose_list'),
					searchURL 		= $choose_list.attr('data-url'),
					filters 		= {'article_title' : $choose_list.find('[name="article_title"]').val()};
				$.get(searchURL, filters, function(data) {
		...
		submit_link_article : function() {
			...
			$('form[name="theForm"]').on('submit', function(e) {
				...
				$('.nav-list-content li').each(function (index){
					...
					info.linked_array.push({'article_id' : article_id});
				});
				$.get(url, info, function(data) {
					ecjia.admin.showmessage(data);
				});
		...
		save_sort : function() {
			$('.save-sort').on('click', function(e) {
				...
				$('.wookmark-goods-photo li').each(function(i) {
					...
					'{img_id : ' + $this.find('[data-toggle="ajaxremove"]').attr('data-imgid') + ', img_original : "' + $this.find('[data-original]').attr('data-original') + '"},': 
					'{img_id : ' + $this.find('[data-toggle="ajaxremove"]').attr('data-imgid') + ', img_original : "' + $this.find('[data-original]').attr('data-original') + '"},';
				});
				info_str += ']}';
				info = eval('(' + info_str + ')');
