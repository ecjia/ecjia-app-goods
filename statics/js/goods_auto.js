	...
	app.goods_auto = {
		...
		search : function () {
			...
			$(".btnSubmit").on('click', function(e) {
				...
				var option 		= {id : id, url : url, msg : msg, name : name, time_val : time_val, time : time}
				
				if (id.length == 0) {
					smoke.alert(noSelectMsg);
				} else if (time_val == '') {
					smoke.alert('请选择商品上下架的时间');
				} else {
					app.goods_auto.batch(option);
				}
			});
		},
		...
		batch : function(options) {
			...
			if(!options.url || options.id.length == 0)return console.log('批量操作缺少参数！');
