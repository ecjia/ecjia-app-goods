    ...
    app.preview = {
        init: function () {
			...
			if ($('goleft') != null) {
				...
				$('goleft').onmouseover = function() {this.src = images_url + '/goleft2.gif'; MyMar=setInterval(goleft, speed);}
				$('goleft').onmouseout = function() {this.src = images_url + '/goleft.gif'; clearInterval(MyMar);}
				$('goright').onmouseover = function() {this.src = images_url + '/goright2.gif'; MyMar=setInterval(goright,speed);}
				$('goright').onmouseout = function() {this.src = images_url + '/goright.gif'; clearInterval(MyMar);}
    ...
    app.goods_attr = {
        init: function () {
            ...
            $('[data-toggle="get_attr_list"]').on('change', function (e) {
                ...
                $.get(url, {goods_type: type}, function (data) {
    ...
    app.link_goods = {
        ...
        load_link_goods_opt: function (data) {
            ...
            if (data.content.length > 0) {
                for (var i = 0; i < data.content.length; i++) {
                    ...
                    var opt = '<li class="ms-elem-selectable ' + disable + '" id="goodsId_' + data.content[i].value +
                        '" data-id="' + data.content[i].value + '" data-price="' + data.content[i].data + '"><span>' + data.content[i].text + '</span></li>'
        ...
        add_link_goods: function () {
            $('.nav-list-ready li').on('click', function () {
                var $this = $(this),
                    tmpobj = $(
                        '<li class="ms-elem-selection"><input type="hidden" name="goods_id[]" data-double="0" data-price="' +
                        $this.attr('data-price') + '" value="' + $this.attr('data-id') +
                        '" /><span class="link_static m_r5">[' + js_lang.single + ']</span>' + $this.text() +
                        '<span class="edit-list"><a class="change_links_mod" href="javascript:;">' + js_lang.change_connect +
                        '</a><i class="fa fa-times ecjiafc-red del"></i></span></li>');
                if (!$this.hasClass('disabled')) {
...
//                    tmpobj.trigger('dblclick');
        ...
        change_link_model: function () {
            ...
            $(document).off('click.changelinksmod').on('click.changelinksmod', '.change_links_mod', function () {
                ...
                $info.attr('data-double') == 1 ?
                    $info.attr('data-double', '0').parents('.ms-elem-selection').find('.link_static').text('[' + js_lang.single + ']') :
    ...
    app.link_parts = {
        ...
        load_link_goods_opt: function (data) {
            ...
            if (data.content.length > 0) {
                for (var i = 0; i < data.content.length; i++) {
                    var disable = $('.nav-list-content .ms-elem-selection').find('input[value="' + data.content[i].value + '"]').length ? 'disabled' : '';
                    var opt = '<li class="ms-elem-selectable ' + disable + '" id="goodsId_' + data.content[i].value + '" data-id="' 
                    	+ data.content[i].value + '" data-price="' + data.content[i].data + '"><span>' +
                        data.content[i].text + '</span></li>'
                    $('.nav-list-ready').append(opt);
                };
            } else {
                $('.nav-list-ready').html('<li class="ms-elem-selectable disabled"><span>' + js_lang.select_goods_empty + '</span></li>');
        ...
        add_link_goods: function () {
            $('.nav-list-ready li')
                ...
                var $this = $(this),
                    tmpobj = $(
                        '<li class="ms-elem-selection"><input type="hidden" name="goods_id[]" data-double="0" data-price="' +
                        $this.attr('data-price') + '" value="' + $this.attr('data-id') + '" />' + $this.text() +
                        '<span class="link_price m_l5">[' + js_lang.price + ':' + $this.attr('data-price') +
                        ']</span><span class="edit-list"><a class="change_link_price" href="javascript:;">' + js_lang.modify_price +
                        '</a><i class="fa fa-times ecjiafc-red del"></i></span></li>');
                if (!$this.hasClass('disabled')) {
                    tmpobj.appendTo($(".ms-selection .nav-list-content"));
                    $this.addClass('disabled');
    ...
    app.link_article = {
        ...
        add_link_article: function () {
            $('.nav-list-ready li')
                .on('click', function () {
                var $this = $(this),
                    tmpobj = $('<li class="ms-elem-selection"><input type="hidden" name="article_id[]" value="' + $this.attr('data-id') + '" />' + $this.text() +
                        '<span class="edit-list"><i class="fa fa-times ecjiafc-red del"></i></span></li>');
                if (!$this.hasClass('disabled')) {
                    tmpobj.appendTo($(".ms-selection .nav-list-content"));
                    $this.addClass('disabled');
                }
        ...
        save_sort: function () {
            $('.save-sort').on('click', function (e) {
                ...
                $('.wookmark-goods-photo li').each(function (i) {
                    var $this = $(this);
                    info_str += i + 1 == $('.wookmark-goods-photo li').length ?
                        '{img_id : ' + $this.find('[data-toggle="ajaxremove"]').attr('data-imgid') +
                        ', img_original : "' + $this.find('[data-original]').attr('data-original') + '"},' :
                        '{img_id : ' + $this.find('[data-toggle="ajaxremove"]').attr('data-imgid') +
                        ', img_original : "' + $this.find('[data-original]').attr('data-original') + '"},';
                });
                info_str += ']}';
