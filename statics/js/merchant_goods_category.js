	...
	app.goods_category_info = {
		...
		choose_goods_type : function() {
			$(document).on('change', '.choose_goods_type', function() {
				...
				// console.log(url);
				val == 0 ?
				$this.parents('.goods_type').find('.show_goods_type').html('<option>请选择筛选属性</option>').trigger("liszt:updated") : 
				$.get(url, {'cat_id' : val}, function(data) {
					var opt = '';
		...
		submit_info : function() {
			...
			var option = {
				rules : {cat_name : {required : true}},
				messages : {cat_name : {required :"请输入分类名称"}},
